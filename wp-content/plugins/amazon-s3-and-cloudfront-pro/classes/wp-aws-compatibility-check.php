	...
	class WP_AWS_Compatibility_Check {
		...
		function get_error_msg() {
			...
			if ( ! version_compare( $this_plugin_version, $this_plugin_version_required, '>=' ) ) {
				...
				$upgrade_msg = ' <a style="font-weight:bold;text-decoration:none;white-space:nowrap;" href="' . $update_url . '">' . sprintf( __( 'Update %s to the latest version', 'as3cf-pro' ), $this->plugin_name ) . '</a>';

				global $as3cfpro;
				if ( ! empty( $as3cfpro ) && $as3cfpro->get_plugin_slug( true ) === $this->parent_plugin_slug ) {
					// Don't show update link for addons of a licensed plugin where the license is invalid
					if ( ! $as3cfpro->is_valid_licence() ) {
						$upgrade_msg = ' ' . sprintf( __( 'A valid license for %s is required to update.', 'as3cf-pro' ), $this->get_parent_plugin_name() );
					}
				}
